  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: 8 }).map((_, i) => (
        <div
          key={i}
          className="flex items-center gap-3 p-3 rounded-lg bg-(--color-surface) border border-(--color-border)"
        >
...
function SourceBadge({ source }: { source: string }) {
  return (
    <span className="px-1.5 py-0.5 text-[10px] font-medium uppercase rounded bg-(--color-border) text-(--color-text-muted) leading-none">
      {source}
    </span>
  );
}
...
function LeaderboardRow({ rank, item }: { rank: number; item: SearchResult }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 p-3 rounded-lg bg-(--color-surface) border border-(--color-border) hover:border-(--color-accent) transition-colors duration-150 group"
    >
      <span
      ...
      {item.cover ? (
        ...
        <div className="w-10 h-14 rounded bg-(--color-border) shrink-0" />
      )}

      <div className="min-w-0 flex-1">
        <p className="text-[13px] font-medium text-(--color-text) line-clamp-1 group-hover:text-(--color-accent) transition-colors duration-150">
          {item.title}
        </p>
      ...
      {item.rating != null && item.rating !== "" && (
        <div className="flex items-center gap-1 shrink-0">
          ...
          <span className="text-[12px] font-medium text-(--color-text)">
            {item.rating}
          </span>
        </div>
      )}
...
export default function LeaderboardPage() {
  ...
  return (
    <div className="space-y-6 max-w-240 mx-auto">
      <SectionErrorBoundary>
        <h1 className="text-xl font-semibold tracking-tight">Leaderboard</h1>

        <div className="flex gap-1 p-1 bg-(--color-surface) rounded-lg border border-(--color-border)">
          {TABS.map((tab) => (
            <button
              key={tab.key}
