  ...
  return (
    <div
      className={`sm:hidden overflow-hidden transition-[max-height] duration-300 ease-in-out border-b border-(--color-border) ${
        menuOpen ? "max-h-[20rem]" : "max-h-0 border-b-0"
      }`}
    >
      <div className="px-4 py-2 flex flex-col gap-1 bg-(--color-bg)">
        {isMangaPage && (
          <button
            onClick={() => { goBack(); closeMenu(); }}
        ...
        <Link href="/whitelist" onClick={closeMenu} className={mobileLinkClass("/whitelist")}>
          Whitelist
        </Link>
        <Link href="/stats" onClick={closeMenu} className={mobileLinkClass("/stats")}>
          Statistik
        </Link>
        <Link href="/dashboard" onClick={closeMenu} className={mobileLinkClass("/dashboard")}>
          Status
        </Link>
        <Link href="/incidents" onClick={closeMenu} className={mobileLinkClass("/incidents")}>
          Insiden
        </Link>
        <Link href="/logs" onClick={closeMenu} className={mobileLinkClass("/logs")}>
          Logs
        </Link>

        <div className="border-t border-(--color-border) mt-1 pt-1 space-y-1">
          <LoginModalButton onTrigger={closeMenu} className="px-4 py-3 text-[15px] font-medium rounded-lg transition-colors duration-150 inline-flex items-center gap-2 w-full text-(--color-text-secondary) hover:text-(--color-text) hover:bg-(--color-surface)">
            Login
          </LoginModalButton>
          <button
