  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[13px] text-[var(--color-text-muted)] mt-0.5">Jelajahi berdasarkan genre</p>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {genres.map((g) => (
            <Link
              key={g.taxonomy_id}
              href={`/genres/${g.slug}`}
              className="rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] px-4 py-3 text-[13px] font-medium text-[var(--color-text-secondary)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)] transition-colors"
            >
              {g.name}
            </Link>
          ))}
