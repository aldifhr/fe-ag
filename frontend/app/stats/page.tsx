...
function StatCard({
  ...
  return (
    <div className="flex flex-col items-center gap-1 p-4 rounded-xl bg-(--color-surface) border border-(--color-border)">
...
export default function StatsPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      {/* Activity chart */}
      <div className="p-5 rounded-xl bg-(--color-surface) border border-(--color-border) mb-6">
            ...
            return (
              ...
              >
                ...
                >
                  {height > 0 ? (
                    <div
                      className={`w-full max-w-8 rounded-t-md transition-all ${isToday ? "bg-(--color-accent)" : "bg-(--color-accent) opacity-60"}`}
                      style={{ height: `${height}%` }}
                    />
                  ) : (
                    <div className="w-full max-w-8 h-0.5 bg-(--color-border)" />
                  )}
                </div>
                <span
                  className={`text-[11px] ${isToday ? "font-bold text-(--color-accent)" : "text-(--color-text-muted)"}`}
                >
                  {dayName}
                </span>
              </div>
      ...
      {/* Today + average */}
      <div className="p-5 rounded-xl bg-(--color-surface) border border-(--color-border)">
