...
function classifyError(message: string): "network" | "timeout" | "server" {
  ...
  if (m.includes("network") || m.includes("fetch") || m.includes("econnrefused") || m.includes("failed to fetch")) return "network";
  if (m.includes("500") || m.includes("server")) return "server";
  return "server";
}

const ERROR_MESSAGES = {
  network: "Tidak dapat terhubung ke server. Pastikan backend berjalan di localhost:3000",
...
export default function ErrorView({
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0f] px-4">
      ...
      {message && (
        <p className="text-[11px] text-[#52525b] mb-6 text-center max-w-md break-all">{message}</p>
      )}
      ...
      {connStatus && (
        <div className="w-full max-w-sm rounded-lg bg-[#14141f] border border-[#27273a] p-4 text-center">
          <p className="text-[12px] text-[#71717a] mb-3 font-medium uppercase tracking-wider">Status Koneksi</p>
          ...
          {!connStatus.backend && (
            <p className="text-[11px] text-red-400/70 mt-3">Pastikan backend berjalan: npm run dev</p>
          )}
          {connStatus.backend && !connStatus.shinigami && (
            <p className="text-[11px] text-yellow-400/70 mt-3">Shinigami sedang tidak tersedia</p>
          )}
        </div>
      )}
