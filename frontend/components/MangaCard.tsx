...
export default function MangaCard({ title, cover, source, id, chapter, time, chapters }: Props) {
  ...
  return (
    <Link
      ...
      className="group block rounded-lg overflow-hidden bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-[var(--color-border-hover)] transition-colors duration-200"
    >
      {/* Cover */}
      ...
      <div className="px-3 py-2.5">
        ...
        <span className="inline-block mt-1.5 px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wider rounded text-[var(--color-text-muted)] bg-[var(--color-surface-hover)]">
          {source}
        </span>
      </div>
    </Link>
