...
export default function SearchModal({ open, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={handleClose}
        className="absolute top-4 right-4 z-10 p-2 rounded-lg text-(--color-text-muted) hover:text-(--color-text) hover:bg-white/5 transition-colors duration-150"
        aria-label="Tutup"
      >
        <svg
          width="20"
      ...
      <div className="shrink-0 px-4 sm:px-6 pb-4">
        <div className="relative max-w-2xl mx-auto w-full">
          ...
          <input
            ...
            className="w-full pl-12 pr-10 py-3.5 rounded-xl bg-white/5 border border-white/10 text-(--color-text) text-[15px] outline-none placeholder:text-(--color-text-muted) focus:border-(--color-accent) transition-colors duration-150"
          />
          {query && (
            <button
              onClick={() => setQuery("")}
            ...
            <div className="max-w-2xl mx-auto flex flex-col">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="flex items-center gap-3 px-3 py-2">
                  <div className="w-9 h-13 skeleton rounded shrink-0" />
          ...
          {!loading && error && (
            <div className="text-center py-16">
              ...
              <button
                ...
                className="px-4 py-2 text-[13px] font-medium rounded-lg bg-white/5 border border-white/10 text-(--color-text-secondary) hover:text-(--color-text) hover:border-white/15 transition-colors duration-150"
              >
                Coba Lagi
              </button>
            </div>
            ...
            <div className="max-w-2xl mx-auto flex flex-col">
              {results.slice(0, MAX_RESULTS).map((item, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[13px] font-medium text-(--color-text) truncate group-hover:text-white transition-colors">
                      {item.title}
                    </p>
                  ...
                  <svg
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="shrink-0 text-(--color-text-muted) opacity-0 group-hover:opacity-100 transition-opacity"
                  >
              ...
              {results.length > MAX_RESULTS && (
                <button
                  ...
                  className="mt-1 py-2.5 text-[13px] font-medium text-(--color-accent) hover:text-(--color-accent-hover) transition-colors duration-150 text-center"
                >
