    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="flex items-center gap-3 p-4 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)]">
          ...
          <p className="text-[13px] text-[var(--color-text-muted)] flex-1">Gagal memuat section ini.</p>
          <button
            onClick={() => this.setState({ hasError: false })}
            className="px-3 py-1 text-[12px] font-medium rounded bg-[var(--color-accent)] text-white hover:opacity-90 transition-opacity"
          >
            Coba lagi
          </button>
        </div>
